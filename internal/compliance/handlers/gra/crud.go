...
func HandleAdminGetPolicy(db database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var rows []map[string]any
		if err := db.QueryRowsCompound(database.TblCorePolicies, database.ColsQCorePolicies, "policy_id", id, "tenant_id", tenantID, &rows); err != nil || len(rows) == 0 {
...
func HandleAdminUpdatePolicy(db database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		respond.LimitBody(r)
		// Previously any JSON key was forwarded directly to core_policies — column injection
		...
		if err := db.UpdateRowCompound(database.TblCorePolicies, "policy_id", id, "tenant_id", tenantID, update); err != nil {
...
// Queries core_policies (not core_rules which is used for /ops/rate-limits).
...
// Creates a new governance policy in core_policies.
...
func HandleAdminCreatePolicy(db database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.QueryRows(database.TblCorePolicies,
