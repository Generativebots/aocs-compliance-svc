...
BEGIN
  IF TG_OP IN ('INSERT') AND NEW.control_id IS NOT NULL THEN
    UPDATE compliance.core_compliance
...
END;
$$;
DROP TRIGGER IF EXISTS trg_evidence_count_sync ON compliance.core_evidence;
CREATE TRIGGER trg_evidence_count_sync
  AFTER INSERT OR DELETE ON compliance.core_evidence
